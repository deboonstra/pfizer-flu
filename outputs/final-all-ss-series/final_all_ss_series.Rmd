...
subtitle: "using the final disease cohort"
...
An analytical approach was devised to estimate the change in incidence for diseases that are seasonally related to influenza. The approach is outlined as we go through this document. The results on this analysis is based on records from 2003 through 2015 based on the ICD-9-CM coding system and included all inpatient and outpatient records.
...
### Convergence issues {.tabset .tabset-fade}
In this analysis, the diseases below had convergence issues that did not allow for the extraction of latent components. The reason for the convergence issue is listed under the *Issue* tab. 
```{r}
all_series_issue <- readRDS(
  file = "./outputs/final-all-ss-series/final_all_series_issue.rds"
)
all_models_issue <- readRDS(
...
#### Issues
```{r}
prob_info <- data.frame(
  ccs = names(all_models_issue),
  issue = unname(sapply(X = all_models_issue, function(x) x))
)
prob_info$ccs <- gsub("ccs", "CCS: ", prob_info$ccs)
prob_info$issue <- gsub("\n", "", prob_info$issue)
tab <- knitr::kable(
  x = prob_info,
  align = c("r", "l", "r", "r", "r"),
  col.names = c("CCS", "Issue")
)
kableExtra::kable_styling(kable_input = tab)
```
...
#### Winter seasonality
A diseases was classified to have winter seasonality if its global correlation with influenza was greater than or equal to 0.60. There were 14 diseases that met this criteria.
```{r}
...
#### Summer seasonality
A diseases was classified to have summer seasonality if its global correlation with influenza was less than or equal to -0.60. There were 46 diseases that met this criteria.
```{r}
...
A diseases was classified to have no seasonality if its global correlation with influenza was less than 0.60 and greater than -0.60. There were 221 diseases that met this criteria.
```{r}
for (j in seq_len(nrow(corr_all))) {
