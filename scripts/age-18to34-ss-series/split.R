...
# combined ICD-9-CM and ICD-10-CM crossover data for the less than 2 age group.
...
if (!dir.exists("./outputs/age-18to34-ss-series/")) {
  dir.create("./outputs/age-18to34-ss-series/")
}
...
saveRDS(
  object = icd_ts,
  file = "./outputs/age-18to34-ss-series/all_series.rds"
)
if (sum(convergence) > 0) {
  saveRDS(
    object = icd_ts_issue,
    file = "./outputs/age-18to34-ss-series/all_series_issue.rds"
  )
}
...
saveRDS(
  object = all_models,
  file = "./outputs/age-18to34-ss-series/all_models.rds"
)
if (sum(convergence) > 0) {
  saveRDS(
    object = all_models_issue,
    file = "./outputs/age-18to34-ss-series/all_models_issue.rds"
  )
}
...
if (length(valid_diseases) < original_len) {
  saveRDS(
    object = bad_diseases,
    file = "./outputs/age-18to34-ss-series/bad_diseases.rds"
  )
}