...
# combined ICD-9-CM and ICD-10-CM crossover data for the 55 to 64 age group.
...
if (!dir.exists("./outputs/age-55to64-ss-series/")) {
  dir.create("./outputs/age-55to64-ss-series/")
}
...
if (sum(convergence) > 0) {
  saveRDS(
    object = icd_ts_issue,
    file = "./outputs/age-55to64-ss-series/all_series_issue.rds"
  )
}
...
if (sum(convergence) > 0) {
  saveRDS(
    ...
    file = "./outputs/age-55to64-ss-series/all_models_issue.rds"
  )
}
...
if (length(valid_diseases) < original_len) {
  saveRDS(
    object = bad_diseases,
    file = "./outputs/age-55to64-ss-series/bad_diseases.rds"
  )
}