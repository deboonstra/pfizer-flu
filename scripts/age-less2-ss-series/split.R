...
if (!dir.exists("./outputs/age-less2-ss-series/")) {
  dir.create("./outputs/age-less2-ss-series/")
}
...
if (sum(convergence) > 0) {
  saveRDS(
    object = icd_ts_issue,
    file = "./outputs/age-less2-ss-series/all_series_issue.rds"
  )
}
...
saveRDS(
  object = all_models,
  file = "./outputs/age-less2-ss-series/all_models.rds"
)
if (sum(convergence) > 0) {
  saveRDS(
    object = all_models_issue,
    file = "./outputs/age-less2-ss-series/all_models_issue.rds"
  )
}
...
if (length(valid_diseases) < original_len) {
  saveRDS(
    object = bad_diseases,
    file = "./outputs/age-less2-ss-series/bad_diseases.rds"
  )
}