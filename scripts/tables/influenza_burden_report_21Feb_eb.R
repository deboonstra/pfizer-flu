...
if (!dir.exists("./tables/influenza-burden-report-21Feb-eb/")) {
  dir.create("./tables/influenza-burden-report-21Feb-eb/")
}
...
## Table 6
## Caption: Disease descriptions for CCS codes identified as being globally
## and locally correlated with influenza, and diseases that peak in the summer.

### Importing all CCS codes ####
tab6 <- utils::read.csv(file = "./data/ccs-series/ccs_labels.csv")

### Pulling the CCS codes used in all tables ####
tab6 <- subset(
  x = tab6,
  subset = ccs_code %in% c(tab2[, 1], tab3[, 1], tab4[, 1], tab5[, 1])
)
tab6 <- tab6[order(tab6$ccs_code), ]
row.names(tab6) <- NULL
